export default function Toast({ message, type, onClose }) {
  const typeStyles = {
    ...
    info: "border-l-4 border-border html.light:border-zinc-400"
  };
  ...
  return (
    <div className={`minimal-card p-4 flex items-center justify-between gap-4 pointer-events-auto min-w-[280px] max-w-sm shadow-lg ${typeStyles[type] || typeStyles.info}`}>
      ...
      <button 
        onClick={onClose}
        className="text-muted-foreground hover:text-muted-foreground transition-colors cursor-pointer text-xs font-bold"
      >
        ✕
      </button>
    </div>
