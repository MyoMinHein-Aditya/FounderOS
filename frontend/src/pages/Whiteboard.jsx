            ...
            <main className="flex-1 min-w-0 pt-20 px-4 md:px-8 lg:px-12 max-w-7xl mx-auto w-full pb-6 flex flex-col h-full">
                <header className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4 flex-shrink-0">
                    ...
                    <div className="flex items-center gap-3.5">
                        ...
                        <input 
                            type="color"
                            className="w-8 h-8 rounded border border-border bg-transparent cursor-pointer p-0.5"
                            value={color}
                            onChange={(e) => setColor(e.target.value)}
                        />
                        <button 
                            className="px-4 py-2 bg-muted border border-border hover:bg-secondary text-muted-foreground font-bold rounded-xl text-xs transition-all cursor-pointer"
                            onClick={clearCanvas}
                        >
                            Clear Board
                        </button>
                ...
                <div className="flex-1 min-h-0 minimal-card p-2 rounded-2xl relative overflow-hidden border border-border bg-[#090d16] mb-4">
                    <canvas 
                        ref={canvasRef}
